...
\begin{frame}{扩张是否唯一?}
\begin{exam}
(\tc{有限集合上的概率}) 设样本空间${\Omega=\{a, b, c, d\}, \sigma}$-代数${\mathcal{F}}$由集族${\mathcal{A}}$生成, 即$\mathcal{F}=\sigma(\mathcal{A}), \quad  \mathcal{A}=\{\{a\},\{a, b\},\{a, b, c\}, \Omega\}$. 定义
...
\begin{frame}{{\rm Boole}不等式证明:非归纳法}
	\begin{itemize}[<+-|alert@+>]
		...
		\begin{align*}
		  P(\cup_{i=1}^nA_i) &=\pause P(A_1)+\sum_{i=2}^nP(A_i\overline{A}_{i-1}\cdots\overline{A}_1) \\
		   ...
		   &=\pause \sum_{i=1}^nP(A_i)-\sum_{i=2}^nP(\cup_{j=1}^{i-1}(A_iA_j))\leq \pause \sum_{i=1}^nP(A_i)
		\end{align*}
	\end{itemize}
	...
	\begin{frame}{{\rm Bonferroni}不等式证明:非归纳法}
	\begin{itemize}[<+-|alert@+>]
		\item 注意到
		{\small \begin{align*}
...
\begin{frame}{配对问题二}
	\begin{exam}
		接上面例子, 试求下述各事件的概率：\\
		(1) 恰有 $k$ 个孩子戴对帽子；(2) 至少有 $m$ 个孩子戴对帽子.
	\end{exam}
	\pause
	%\vspace{0.4cm}
	\begin{itemize}[<+-|alert@+>]
		\item 记 $E_k=$\{恰有 $k$ 个孩子戴对帽子 \},$A_m=$\{至少有 $m$ 个孩子戴对帽子 \};
