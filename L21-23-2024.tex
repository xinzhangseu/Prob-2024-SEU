...
\item 故由 $P\left(A_{k}\right)=E\left(1_{A_k}\right)$ 及 $A_{k}$ 的定义可推出
	...
	\item 注意到 $S_{k}^{2} 1_{A_k}$ 是 $\xi_{1}, \cdots, \xi_{k}$ 的函数，它与 $S_{n}-S_{k}=\sum_{i=k+1}^{72} \xi_{i}$ 相互独立. 故
	\[
...
\begin{frame}{Kolmogorov强大数定律证明}
	\begin{itemize}[<+-|alert@+>]
		...
		\item 令$E=\bigcup_{\ell=1}^{\infty} E_{\ell}$, 则 ${P(E)=0}$.
		\item 对任意的$\omega\notin E$均有 ${b(\omega) \leq 1 / \ell}, \forall l\geq 1$.
		\item 故对任意的 $\omega \notin {E}$ 恒有 $b(\omega)=0$.
		\item 从而, 在${\bar{E}}$ 上 ${\sum_{k=1}^{\infty} X_{k}'/ k}$ 收敛.
		\item 故在 ${\bar{E}}$ 上 ${\lim _{n \rightarrow \infty} \dfrac{1}{n} \sum_{k=1}^{n} X_{k}'=0}$.
